# Run GP benchmark on CPU (single thread)
library(here)
...
REG_PATH <- here("benchmarks", "gp", paste0("registry-cpu-single-", length(parallel::mcaffinity())))
...
saveRDS(result, here("benchmarks", "gp", paste0("result-cpu-single-", length(parallel::mcaffinity()), ".rds")))
